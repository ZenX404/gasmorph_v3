...
已登录用户可主动断开钱包连接，并在登录后看到简要的项目摘要模块与后续操作入口。
...
2. **Given** 用户已登录，**When** 查看首屏摘要模块，**Then** 能看到项目核心要点及下一步行动入口（如查看详情/联系按钮）。
...
- **FR-001**: 系统必须提供包含项目价值主张及“连接钱包”主按钮的落地页。
...
- **FR-007**: 系统必须在登录后展示简要的项目摘要模块，并提供后续行动入口（如查看详情、联系）。
...
- **SC-004**: 登录后至少 80% 的访客能理解项目核心要点并找到后续行动入口（通过可用性反馈或问卷）。
