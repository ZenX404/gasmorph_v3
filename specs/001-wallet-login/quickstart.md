...
4) 可选：本地链 `anvil`（Foundry）用于 E2E/模拟，命令：`anvil --chain-id 1337 --port 8545`。默认会生成预置账户并附带测试 ETH，例如首个账户地址 `0xf39fd6e51aad88f6f4ce6ab8827279cfffb92266`（私钥为 anvil 默认测试密钥，请勿在真实网络使用）。
...
## 钱包/网络说明
- 默认支持：Sepolia (chainId 11155111)、Monad 测试网 (chainId 20143，若官方有更新请同步)、本地 anvil (chainId 1337)。
- 不支持网络时，会提示切换，并提供切换到 Sepolia/Monad 的按钮；拒绝授权会保持未登录并提示错误。
- 移动端：如需调用钱包 App，可使用钱包自带的深链；若浏览器扩展不可用，会提示连接失败或需切换到支持的环境。
